...
|Address|Name                     |R/W|Description                                                                                                  |
|-------|-------------------------|---|-------------------------------------------------------------------------------------------------------------|
|0x2C00 |主机故障字               | R |bit0:主机系统故障 bit1二次侧回水传感器故障 bit3空调外机故障    bit4进风风机故障      bit5回风风机故障        |
|       |                         |   |bit6户外进风传感器故障 bit7氟盘前传感器故障 bit8氟盘后传感器故障  bit9回风传感器故障 bit10排风传感器故障     |
|       |                         |   |bit11 一次侧供水传感器故障 bit12一次侧回水传感器故障 bit13排水故障                                           |
|0x2C01 |预冷(预热)盘输出         |R/W|不输出：0x0000   输出：0x0001  调试使能开启时可控制状态                                                      |
|0x2C02 |系统开关机               |R/W|关机:0x0000  开机:0x0001                                                                                     |
|0x2C03 |系统运行模式             |R/W|制冷模式:0x0001;制热模式:0x0002;除湿模式:0x0003;新风模式:0x0004;夏季自动:0x0005;                             |
|       |                         |   |冬季自动:0x0006;热水模式:0x0007;                                                                             |
|0x2C04 |设定相对湿度             |R/W|INT16U,0x0020 表示：当前运行设定湿度为32%      范围10%到90%  精度1%                                          |
|0x2C05 |通风设定风速             |R/W|低风:0x0001;中风:0x0002;高风:0x0003;                                                                         |
|0x2C06 |回风制冷设定温度         |R/W|INT16U,0x00FF 分辨率0.5   (步长)0.5度 当前制冷设定温度是26.0度，范围10到35度                                 |
|0x2C07 |回风制热设定温度         |R/W|INT16U,0x00FF 分辨率0.5   (步长)0.5度 当前制热设定温度是20.0度，范围10到35度                                 |
...
|0x2C0E |主机制冷模式设定供水水温 |R/W|INT16U,0x0014 分辨率0.5   (步长)0.5度 当前温度是20.0度   范围7度到55度                                       |
|0x2C0F |主机制热模式设定供水水温 |R/W|INT16U,0x0019 分辨率0.5   (步长)0.5度 当前温度是25.0度   范围7度到55度                                       |
|0x2C10 |主机生活热水设定水温     |R/W|INT16U,0x00FF 分辨率0.5   (步长)0.5度 当前温度是25.0度   范围40度到55度                                      |
|0x2C11 |户外进风口温度           | R |INT16S,0x00FA 表示：当前进风口温度是25.0度                                                                   |
|0x2C12 |户外进风口湿度           | R |INT16U,0x0020 表示：当前进风口湿度是32%                                                                      |
|0x2C13 |氟盘前温度               | R |INT16S,0x00FA 表示：当前进风口温度是25.0度                                                                   | 
|0x2C14 |氟盘前湿度               | R |INT16U,0x0020 表示：当前进风口湿度是32%                                                                      | 
|0x2C15 |氟盘后温度               | R |INT16S,0x00FA 表示：当前进风口温度是25.0度                                                                   | 
|0x2C16 |氟盘后湿度               | R |INT16U,0x0020 表示：当前进风口湿度是32%                                                                      | 
...
|0x2C24 |主机生活热水测量温度     | R |INT16S,0x00FA 表示：当前温度是25.0度                                                                         | 
|0x2C25 |故障复位字               | W |写入1有效  bit0:复位排水故障;  Bit15:复位滤网使用时间;                                                       | 
...
|0x2C3C |输配一次侧供水温度       |R/W|INT16S,0x00FA 表示：当前进风口温度是25.0度                                                                   |
|0x2C3D |输配一次侧回水温度       |R/W|INT16S,0x00FA 表示：当前进风口温度是25.0度                                                                   |
...
|0x2C48 |制冷无能需出水温度配置   |R/W|基本策略配置用                                                                                               |    
|0x2C49 |制冷风盘能需出水温度配置 |R/W|基本策略配置用                                                                                               |    
|0x2C4A |制冷辐射能需出水温度配置 |R/W|基本策略配置用                                                                                               |    
|0x2C4B |制冷露点比较温度配置     |R/W|基本策略配置用                                                                                               |    
|0x2C4C |新风效率                 | R |进回风温度效率                                                                                               |    
|0x2C4D |风机切换                 |R/W|0x0000：不切换; 0x0001：切换 开启此功能后，风机在硬件逻辑上互换                                              |    
|0x2C4E |热水使能                 | W |0x0000：关闭; 0x0001：开启                                                                                   |    
|0x2C4F |制冷回水设置温度         |R/W|INT16U,0x0014 分辨率0.5   (步长)0.5度 当前温度是20.0度   范围7度到55度                                       |    
|0x2C50 |滤芯更换周期             |R/W|INT16U,0x0010 表示: 当前滤网更换周期为16天范围1-720天(2年                                                    |    
|0x2C51 |制热回水设置温度         |R/W|INT16U,0x0014 分辨率0.5   (步长)0.5度 当前温度是20.0度   范围7度到55度                                       |   
|0x2C52 |滤芯使用时间             |R/W|INT16U,0x0020 表示: 当前滤网已使用32天写入只能是0x0000，其他数据忽略                                         |    
|0x2C53 |制热无能需出水温度配置   |R/W|基本策略配置用                                                                                               |    
|0x2C54 |制热风盘能需出水温度配置 |R/W|基本策略配置用                                                                                               |    
|0x2C55 |制热辐射能需出水温度配置 |R/W|基本策略配置用                                                                                               |    
|0x2C56 |制热回风比较温度配置     |R/W|基本策略配置用                                                                                               |    
|0x2C57 |除湿固定湿度配置         |R/W|基本策略配置用                                                                                               |    
|0x2C58 |制冷氟盘最大设定温度     |R/W|基本策略配置用                                                                                               |    
|0x2C59 |制热氟盘最小设定温度     |R/W|基本策略配置用                                                                                               |    
|0x2C5A |制热氟盘最大设定温度     |R/W|基本策略配置用                                                                                               |    
...
|0x2C61 |外机水机开关机           |R/W|0x0000:关机        0x0001:开机                                                                               |    
